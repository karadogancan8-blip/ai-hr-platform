...
export function HrDocsAndAppeal({ storageKey, subjectLabel }: HrDocsAndAppealProps) {
  const { t } = useI18n();
  ...
  function ingest(list: FileList | null) {
    ...
    const accepted = Array.from(list).filter((file) => /pdf|image|jpeg|png|webp|jpg/i.test(file.type) || /\.(pdf|png|jpe?g|webp)$/i.test(file.name));
    if (!accepted.length) {
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-1 text-lg font-semibold text-slate-900">{t("appeal.title")}</h3>
                <p className="mt-1 text-sm text-slate-500">{t("appeal.lead")}</p>
              </div>
