...
export function InfoTooltip({ text, side = "top", align = "start", sideOffset = 6 }: InfoTooltipProps) {
  const id = useId();
  ...
  return (
    <span className="relative inline-flex shrink-0 align-middle">
      <button
        ...
        className="inline-flex h-5 w-5 cursor-help items-center justify-center rounded-full border border-indigo-100/80 bg-indigo-50/80 text-xs font-semibold text-indigo-600 transition-all hover:scale-105 hover:bg-indigo-100"
      >
...
export function InfoTitle({ children, hint }: { children: ReactNode; hint: string }) {
  return (
    <span className="inline-flex min-w-0 max-w-full items-center gap-2 text-start">
      <InfoTooltip text={hint} side="top" align="start" sideOffset={6} />
